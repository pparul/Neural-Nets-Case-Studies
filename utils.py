...
def create_dataset(n_samples, centers, n_features, cluster_std, random_state):
    """
    Creates a dataset for a multi-class classification problem

    Args:
        n_samples (_type_): _description_
        centers (_type_): _description_
        n_features (_type_): _description_
        cluster_std (_type_): _description_
...
def plot_contour_plots(
    w1_vals, w2_vals, weight_history, func_vals, alpha_choice
):
    ...
    for j in range(len(weight_history)):
        w_val = weight_history[
            j
        ]  # the results from Gradient descent and starting
        # from an initial value
        plt.scatter(
            w_val[0], w_val[1], linestyle="--", marker="o", s=50, c="r"
        )
...
def gradient_descent(g, alpha_choice, max_its, w):
    ...
    weight_history.append(w)
    # compute final cost function value via g
